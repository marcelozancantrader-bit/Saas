...
export function runZoneamentoChecks(input: ZoneamentoInput): NbrFinding[] {
  ...
  if (input.area_terreno_m2 > 0 && input.area_construida_total_m2 !== null) {
    ...
    // Taxa de ocupação — usa área do pavimento térreo (aprox = total / pavimentos)
  ...
  if (zona.altura_max_m !== null) {
    // Heurística: cada pavimento ≈ 3m de pé-direito + entrepiso
  ...
  // Recuos — sempre warn (precisamos medir no projeto)
  findings.push({
